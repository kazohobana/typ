...
class PaymentLink extends Model
{
    ...
    public function getPrecisionAttribute(){
        $sPrecision = get_wallet_precision($this->currency_info);
       return $sPrecision;
    }
    ...
    public function getAmountCalculationAttribute(){
        ...
        if(!empty($price)){
            $amount = get_amount($price,null,$sPrecision) . ' ('.$qty.')';
        }else{
            if($limit == 1){
                $amount = get_amount($min_amount,null,$sPrecision) . ' - ' . get_amount($max_amount,null,$sPrecision);
            }else{
                $amount = 'Unlimited';
            }
        }
