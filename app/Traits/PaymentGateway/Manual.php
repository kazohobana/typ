...
use ControlDynamicInputFields,TransactionAgent;
            ...
            if(userGuard()['type'] == "USER"){
                ...
                if( $basic_setting->sms_notification == true){
                    try{
                        ...
                        sendSms($user, 'ADD_MONEY_REQUEST', [
                            'amount' =>  get_amount($output['amount']->requested_amount,$output['amount']->receiver_cur_code,$output['amount']->wallet_precision),
                            'method' => @$output['currency']['name'],
                            'charge' => get_amount($output['amount']->total_charge,$output['amount']->sender_cur_code,$output['amount']->gateway_precision),
                            'trx' =>  $trx_id,
                        ]);
                    }catch(Exception $e){}
                }
    ...
    public function manualPaymentConfirmedApi(Request $request){
        ...
        $gateway_request = ['currency' => $payment_gateway_currency->alias, 'amount'  => $hasData->data->amount->requested_amount,'wallet_currency'=>$hasData->data->amount->receiver_cur_code];
