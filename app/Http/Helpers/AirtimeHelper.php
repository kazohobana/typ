...
class AirtimeHelper{
   ...
   /**
    ...
    /**
     * get all billers promotions
     */
    public function getPromotions($iso=null,$size = null):array
    {

        if(!$this->access_token) $this->accessToken();
        $access_token = $this->access_token;
        $base_url = $this->config['request_url'];
        $request_endpoint = $base_url .'/promotions'."?size=".$size."&page=1&languageCode=".$iso;
