...
class MoneyExchangeController extends Controller
{
    public function index(Request $request) {
        $transactions = Transaction::merchantAuth()->where([ 'type' => PaymentGatewayConst::TYPEMONEYEXCHANGE])->latest()->take(10)->get()->map(function($item){
            $statusInfo = [
                "success" =>      1,
                "pending" =>      2,
                "rejected" =>     3,
    ...
    public function adminNotification($data,$trx_id,$user){
        $notification_content = [
            ...
            'greeting' => __("Exchange Money")." (".authGuardApi()['type'].")",
            ...
            //push notification
            'push_title' => __("Exchange Money")." (".authGuardApi()['type'].")",
            ...
            'admin_db_title' => "Virtual Card Buy"." (".authGuardApi()['type'].")",
