    ...
    public function moneyOutInfo(){
        $transactions = Transaction::auth()->moneyOut()->latest()->take(5)->get()->map(function($item){
            ...
            return[
                ...
                'will_get'                  => isCrypto($item->details->charges->will_get ?? $item->payable,withdrawCurrency($item)['gateway_currency'],$item->currency->gateway->crypto),
    ...
    public function insertRecordManual($moneyOutData,$gateway,$get_values,$reference,$status) {
        ...
        try{
            $id = DB::table("transactions")->insertGetId([
                ...
                'payable'                       => $moneyOutData->charges->payable,
