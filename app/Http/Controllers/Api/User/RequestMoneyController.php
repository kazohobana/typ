...
class RequestMoneyController extends Controller
{
    ...
    public function index()
    {
        ...
        $transactions = Transaction::auth()->requestMoney()->latest()->take(10)->get()->map(function($item){
            $statusInfo = [
                ...
                if($item->attribute == payment_gateway_const()::SEND){
                    return[
                        'id' => $item->id,
                        'title' => __("Request Money to")." @" . $item->details->receiver_username,
                        'trx' => $item->trx_id,
                        ...
                        'payable_amount' => '',
                        'total_charge' => "",
                        'will_get' =>  get_transaction_numeric_attribute_request_money($item->attribute).' '.get_amount($item->details->charges->receiver_amount,$item->details->charges->receiver_currency,get_wallet_precision($item->creator_wallet->currency)),
                    ...
                    return[
                        'id' => $item->id,
                        'title' => __("Request Money from")." @" . $item->details->sender_username,
                        'trx' => $item->trx_id,
                        'attribute' => $item->attribute,
                        'type' => $item->type,
                        'request_amount' => get_amount($item->request_amount,$item->creator_wallet->currency->code,get_wallet_precision($item->creator_wallet->currency)),
                        'payable_amount' =>   get_transaction_numeric_attribute_request_money($item->attribute).' '. get_amount($item->payable,$item->creator_wallet->currency->code,get_wallet_precision($item->creator_wallet->currency)),
        ...
        $get_remaining_fields = [
            'transaction_type'  =>  PaymentGatewayConst::REQUESTMONEY,
    ...
    public function submit(Request $request) {
        ...
        try{
           ...
           if( $basic_setting->email_notification == true){
            ...
            $notifyDataSender = [
                'trx_id'            => $trx_id,
                'title'             => __("Request Money to")." @" . @$receiver->username." (".@$receiver->fullname.")",
            ...
            $notifyDataReceiver = [
                'trx_id'            => $trx_id,
                'title'             => __("Request Money from")." @" . @$sender_wallet->user->username." (".@$sender_wallet->user->fullname.")",
                'request_amount'    => get_amount($charges['request_amount'],$charges['sender_currency'],$charges['precision_digit']),
                'payable'           => get_amount($charges['payable'],$charges['receiver_currency'],$charges['precision_digit']),
                'charges'           => get_amount( $charges['total_charge'],$charges['receiver_currency'], $charges['precision_digit']),
                'will_get'          => get_amount( $charges['request_amount'],$charges['sender_currency'], $charges['precision_digit']),
    ...
    public function checkUser(Request $request)
    {
        ...
        $data =[
            'user_info'   => $user->email??$user->full_mobile,
        ];
    ...
    public function logLists()
    {
        ...
        if($item->attribute  == PaymentGatewayConst::SEND){
            ...
            $charge = get_amount($item->details->charges->total_charge,$item->creator_wallet->currency->code,get_wallet_precision($item->creator_wallet->currency));
            $payable = get_transaction_numeric_attribute_request_money($item->attribute).' '.get_amount($item->payable,$item->creator_wallet->currency->code,get_wallet_precision($item->creator_wallet->currency));
            $action = true;
        }
        $status_info = [
            '0' => 'Default',
        ...
        return [
            ...
            'title' => $title,
            'request_amount' => get_transaction_numeric_attribute_request_money($item->attribute).' '.get_amount($item->request_amount,$item->creator_wallet->currency->code,get_wallet_precision($item->creator_wallet->currency)),
            'charge' => $charge,
            'payable' => $payable,
            'status' => $item->status,
